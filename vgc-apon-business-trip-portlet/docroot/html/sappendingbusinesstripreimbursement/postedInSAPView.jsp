...
int totalpostinsap=(int)BusinessTripReimbursementLocalServiceUtil.findCountByS_U_SAP(0, ticketNo,applicant, 1, startDate,endDate,sapDocumentId);
%>
		...
		<div class="searchbox">
			...
			<liferay-ui:icon iconCssClass="icon-search"
						...
						url='<%="javascript:" + renderResponse.getNamespace()
							+ "queryBusinessTripReimbursementPostedSAP();"%>' />
...
<aui:form name="searchFm2" style="width:100%">
...
<aui:script>
	function <portlet:namespace/>queryBusinessTripReimbursementPostedSAP() {
	...
	function paymentCompletedUpdateSAPStatus(businessTripReimbursementIds) {
		AUI()
				.use(
						'aui-io-request',
						function(A) {
							var ajaxRequest = A.io
									.request(
											'<%=ajaxURL %>'
													...
													+ '&<portlet:namespace/>businessTripReimbursementIds='
													+ businessTripReimbursementIds,
											{
												autoLoad : false,
												dataType : 'json',
												on : {
