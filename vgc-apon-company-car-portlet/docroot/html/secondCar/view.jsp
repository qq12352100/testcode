...
<%
	SecondCompanyCar secondCompanyCar = null;
	long secondCompanyCarId = ParamUtil.getLong(request, "secondCompanyCarId");
	...
	if (secondCompanyCarId > 0) {
		secondCompanyCar = SecondCompanyCarLocalServiceUtil.fetchSecondCompanyCar(secondCompanyCarId);
		if(secondCompanyCar!=null) {
			isNew = false;
		}
		secondCompanyCar = secondCompanyCar!=null?secondCompanyCar:SecondCompanyCarLocalServiceUtil.createSecondCompanyCar(secondCompanyCarId);
	}else {
		secondCompanyCarId = CounterLocalServiceUtil.increment(SecondCompanyCar.class.getName(),1);
		secondCompanyCar = SecondCompanyCarLocalServiceUtil.createSecondCompanyCar(secondCompanyCarId);
	}
	...
	if(isNew) {
		...
		if(sapUser!=null) {
			...
			secondCompanyCar.setStaffCode(Long.valueOf(sapUser.getStaffCode()));
			...
			secondCompanyCar.setWorkingLocation(sapUser.getWorkLocation());
			secondCompanyCar.setEmail(sapUser.getEmail());
			secondCompanyCar.setOfficeSite(sapUser.getOfficeSite());
			secondCompanyCar.setMobilePhone(sapUser.getCellPhone());
			secondCompanyCar.setOfficePhone(sapUser.getContactNo());
			secondCompanyCar.setContractEndDate(sapUser.getContractEndDate());
			secondCompanyCar.setDrivingLicenseInformation(-1);	
		...
		if(isApplicantAgent!=secondCompanyCar.getIsApplicantAgent()&& Validator.isNotNull(isClickAgent)) {		
			long staffCode = isApplicantAgent?ParamUtil.getLong(renderRequest, "staffCode", 0):themeDisplay.getUser().getFacebookId();	
			staffCode = staffCode==0?secondCompanyCar.getStaffCode():staffCode;
			SAPUser sapUser = SAPUserLocalServiceUtil.getSapUserByStaffCode(String.valueOf(staffCode));
			if(sapUser!=null) {
				secondCompanyCar.setIsApplicantAgent(isApplicantAgent);
				secondCompanyCar.setCompanyName(sapUser.getSapCompanyName());
	...
	String employeeGroupId = secondCompanyCar!=null?secondCompanyCar.getEmployeeGroupId():"";
	String imageQueryPath = themeDisplay.getPathThemeImages()+"/diff/vgc_apon_my_applications_details.png";
	List<AuditTrailLog> auditTrailLogs = AuditTrailLogLocalServiceUtil.findAuditTrailByWF_O_O(secondCompanyCarId, SecondCompanyCar.class.getName());
	List<FileManagement> fileManagements = FileManagementLocalServiceUtil.getByAssetClassNameAndAssetPrimaryKey(SecondCompanyCar.class.getName(),secondCompanyCarId);
...
<portlet:renderURL var="viewSecondCompanyCartURL" />
<portlet:actionURL var="addSecondCompanyCarURL" windowState="normal" />
...
<aui:model-context bean="<%=secondCompanyCar%>"
	model="<%=SecondCompanyCar.class%>" />

<aui:script>
	//When submit the form, there will be a workflow to enabled
	function <portlet:namespace />submitSecondCompanyCar() {
	...
	function <portlet:namespace />saveSecondCompanyCar() {
		if(!validateNotNull('email',1)) {
			return false;
		}else if(!validateEmailFormat(document.getElementById("email").value)) {
			alert('<liferay-ui:message key="vgc.apon.email.format.is.not.right" />');
		...
		<portlet:namespace />setFileInfo();
		document.<portlet:namespace />fm1.<portlet:namespace />expectedCarModel2.value = document.getElementById('<portlet:namespace />'+'expectedCarModel').value;
		document.<portlet:namespace />fm1.<portlet:namespace /><%= Constants.CMD %>.value = "<%= (secondCompanyCar == null || secondCompanyCar.getSecondCompanyCarId()==0 ) ? Constants.ADD : Constants.UPDATE %>";
		submitForm(document.<portlet:namespace />fm1);			
	}
	...
	function validateNotChooseOfDrivingLicenseInformation() {
		...
		if(!isChoose) {
			...
			var surfix = '<liferay-ui:message key="vgc.apon.second.company.car.driving.license.information" />';
...
<!-- Second company car applicant page -->
<aui:form name="fm1" method="POST" action="<%=addSecondCompanyCarURL%>"
	enctype="multipart/form-data">
	<aui:fieldset>
		<aui:input type="hidden" name="redirect" value="<%=redirect%>" />
		<aui:input name="<%=Constants.CMD%>" type="hidden" />
		<aui:input type="hidden" name="secondCompanyCarId"
			value='<%=secondCompanyCarId%>' id="secondCompanyCarId"/>
	...
	<div class="mainbody">
		<div class="topButton">
			<aui:button-row>
				<aui:button type="button" value="vgc.apon.button.company.car.save"
					onClick='<%= renderResponse.getNamespace() + "saveSecondCompanyCar()" %>' />
				<aui:button type="button" value="vgc.apon.button.submit"
					onClick='<%= renderResponse.getNamespace() + "submitSecondCompanyCar()" %>' />
		...
		<div class="tabTitle">
			<h3>
				<liferay-ui:message
					key="vgc.apon.second.company.car.ordering.replacement.form" />
			</h3>
			<div class="Ltext"></div>
			<div class="tickerNo">
				<liferay-ui:message key="vgc.apon.second.company.car.ticket.no" />
				<span class="num"><%=secondCompanyCar.getTicketNo()%></span>
			</div>
			<div class="clear"></div>
		</div>
		<div class="gernal employeeInfo">
			<div class="subtitle">
				<liferay-ui:message
					key="vgc.apon.second.company.car.employee.information" />
				<%if(isHasAppliantAgent) {
				%>
				<input type="checkbox" name="<portlet:namespace/>isApplicantAgent"
					value='<%=true %>' <%=secondCompanyCar.getIsApplicantAgent()?"checked='checked'":"" %>
			...
			<ul>
				<li class="md01"><liferay-ui:message
						key="vgc.apon.second.company.car.name" /></li>
				<li class="md02"><input type="text"
					name="<portlet:namespace/>name" disabled="disabled" maxlength="75"
					value="<%=secondCompanyCar.getName()%>"></li>
				<li class="md01"><liferay-ui:message
						key="vgc.apon.second.company.car.staff.code" /></li>
				<li class="md022">
					<input id="<portlet:namespace />staffCode" type="text" 
						name="<portlet:namespace/>staffCode" readonly="readonly" maxlength="75"
						value="<%=secondCompanyCar.getStaffCode()%>" />
				</li>
			</ul>
			<ul>
				<li class="md01"><liferay-ui:message
						key="vgc.apon.second.company.car.company" /></li>
				<li class="md02"><input type="text"
					name="<portlet:namespace/>companyName" maxlength="75"
					value='<%=secondCompanyCar.getCompanyName()%>' disabled="disabled">
				</li>
				<li class="md01"><liferay-ui:message
						key="vgc.apon.second.company.car.division" /></li>
				<li class="md022"><input type="text"
					name="<portlet:namespace/>division" disabled="disabled" maxlength="75"
					value="<%=secondCompanyCar.getDivision()%>"></li>
			</ul>
			<ul>
				<li class="md01"><liferay-ui:message
						key="vgc.apon.second.company.car.department" /></li>
				<li class="md02"><input type="text"
					name="<portlet:namespace/>department" disabled="disabled" maxlength="75"
					value="<%=secondCompanyCar.getDepartment()%>"></li>
				<li class="md01"><liferay-ui:message
						key="vgc.apon.second.company.car.cost.center" /></li>
				<li class="md022"><input type="text"
					name="<portlet:namespace/>positionCostCenterId" disabled="disabled" maxlength="75"
					value="<%=secondCompanyCar.getPositionCostCenterId()%>"></li>
			</ul>
			<ul>
				<li class="md01"><liferay-ui:message
						key="vgc.apon.second.company.car.contract.type" /></li>
				<li class="md02"><input type="text" maxlength="75"
					name="<portlet:namespace/>employeeGroupId" disabled="disabled"
					value="<%=ContractTypeEnum.getName(secondCompanyCar.getEmployeeGroupId())%>" />
				</li>
				<li class="md01"><liferay-ui:message
						key="vgc.apon.second.company.car.working.contract.end.date" /></li>
				<li class="md022"><input type="text"
					name="<portlet:namespace/>contractEndDate" disabled="disabled" maxlength="75"
					value="<%=secondCompanyCar.getContractEndDate()==null?"":sdf_dmy.format(secondCompanyCar.getContractEndDate())%>"></li>
			</ul>
			<ul>
				<li class="md01"><liferay-ui:message
						key="vgc.apon.second.company.car.work.location" /></li>
				<li class="md02"><select
					name="<portlet:namespace/>workingLocation">
						<%
									String workingLocation = secondCompanyCar.getWorkingLocation();
				...
				<li class="md01"><liferay-ui:message
						key="vgc.apon.second.company.car.office.site" /></li>
				<li class="md022"><select name="<portlet:namespace/>officeSite">
						<%
									String officeSite = secondCompanyCar.getOfficeSite();
			...
			<ul>
				<li class="md01"><liferay-ui:message
						key="vgc.apon.second.company.car.mobile.phone" /></li>
				<li class="md02"><input type="text"
					id="<portlet:namespace/>mobilePhone"
					name="<portlet:namespace/>mobilePhone" maxlength="75"
					value="<%=secondCompanyCar.getMobilePhone()%>" required="required"></li>
				<li class="md01"><liferay-ui:message
						key="vgc.apon.second.company.car.office.phone" /></li>
				<li class="md022"><input type="text"
					name="<portlet:namespace/>officePhone" maxlength="75"
					value="<%=secondCompanyCar.getOfficePhone()%>" required="required" /></li>
			</ul>
			<ul>
				<li class="md01"><liferay-ui:message
						key="vgc.apon.second.company.car.email" /></li>
				<li class="md02"><input type="text"
					name="<portlet:namespace/>email"  id="email" maxlength="75"
					value="<%=secondCompanyCar.getEmail()%>" required="required"></li>
				<li class="md01"></li>
				<li class="md022"></li>
			</ul>
			<ul>
				<li class="md01"><liferay-ui:message
						key="vgc.apon.second.company.car.driving.license.information" /></li>
				<li class="md02"><input type="radio"
					name="<portlet:namespace/>drivingLicenseInformation" value="0"
					<%=secondCompanyCar.getDrivingLicenseInformation()==0?"checked='checked'":"" %>
					onclick="<portlet:namespace/>showUploadWarnWindow(this)"> <liferay-ui:message
						key="vgc.apon.second.company.car.valid.chinese.drive.license" /></li>
				<li class="md01"></li>
				<li class="md022"></li>
			</ul>
			<ul>
				...
				<li class="md02"><input type="radio"
					...
					<%=secondCompanyCar.getDrivingLicenseInformation()==1?"checked='checked'":"" %>>
					<liferay-ui:message
						key="vgc.apon.second.company.car.private.driver" /></li>
				<li class="md01"></li>
				<li class="md022"></li>
			</ul>
			<ul>
				...
				<li class="md02"><input type="radio"
					...
					<%=secondCompanyCar.getDrivingLicenseInformation()==2?"checked='checked'":"" %>>
					<liferay-ui:message
						key="vgc.apon.second.company.car.chinese.driving.license.not.yet.obtained" />
				</li>
				<li class="md01"><liferay-ui:message
						key="vgc.apon.second.company.car.estimated.obtaining.date" /></li>
				<li class="md022"><input
					id="<portlet:namespace/>estimatedObtainingDate" class="Wdate"
					type="text" maxlength="75" 
					value="<%=secondCompanyCar.getEstimatedObtainingDate()!=null?sdf_dmy.format(secondCompanyCar.getEstimatedObtainingDate()):"" %>"
				...
				<ul>
					...
					<li class="md03 remark"><liferay-ui:message
							key="vgc.apon.second.company.car.car.rental.fee.info" /></li>
				</ul>
				<ul class="txt">
					<li class="md01 warning"><liferay-ui:message
							key="vgc.apon.second.company.car.if.apply.before.rehiring.please.add.comments" /></li>
					<li class="md03">						
						<textarea rows="4"
						onblur="if (value ==''){value='If you plan to change the working location during your assignment, please list the different working locations including the working duration at each location.'}"
							...
							name="<portlet:namespace/>comments" required="required" style="font-style: italic;"><%=Validator.isNull(secondCompanyCar.getComments())?"If you plan to change the working location during your assignment, please list the different working locations including the working duration at each location.":secondCompanyCar.getComments()%></textarea>
					</li>
				</ul>
		</div>
		<div class="gernal carInfo">
			<div class="subtitle">
				<liferay-ui:message
					key="vgc.apon.second.company.car.current.car.information" />
			</div>
			<ul>
				<li class="md01"><liferay-ui:message
						key="vgc.apon.second.company.car.current.car.car.model" /></li>
				<li class="md02"><input type="text"
					name="<portlet:namespace/>currentCarModel" maxlength="75"
					value="<%=secondCompanyCar.getCurrentCarModel()%>"></li>
				<li class="md01"><liferay-ui:message
						key="vgc.apon.second.company.car.current.car.color" /></li>
				<li class="md022"><input type="text"
					name="<portlet:namespace/>currentCarColor" maxlength="75"
					value="<%=secondCompanyCar.getCurrentCarColor()%>"></li>
			</ul>
			<ul>
				<li class="md01"><liferay-ui:message
						key="vgc.apon.second.company.car.current.car.plate.number" /></li>
				<li class="md02"><input type="text"
					name="<portlet:namespace/>currentPlateNumber" maxlength="75"
					value="<%=secondCompanyCar.getCurrentPlateNumber()%>"></li>
				<li class="md01"><liferay-ui:message
						key="vgc.apon.second.company.car.current.car.acquiring.date" /></li>
				<li class="md022"><input id="<portlet:namespace/>acquiringDate"
					class="Wdate" type="text" name="<portlet:namespace/>acquiringDate" 
					value="<%=secondCompanyCar.getAcquiringDate()!=null?sdf_dmy.format(secondCompanyCar.getAcquiringDate()):"" %>"
					onFocus="WdatePicker({lang:'en',dateFmt:'dd.MM.yyyy'})" /></li>
			</ul>
		</div>
		<div class="gernal expectedCompy">
			<div class="subtitle">
				<liferay-ui:message
					key="vgc.apon.second.company.car.expected.company.car.infomation" />
			</div>

			<ul>
				<li class="md01"><liferay-ui:message
						key="vgc.apon.second.company.car.expected.car.car.model" /></li>
				<li class="md02"><input type="text"
					id="<portlet:namespace/>expectedCarModel"
					name="<portlet:namespace/>expectedCarModel"
					value="<%=secondCompanyCar.getExpectedCarModel()%>" maxlength="75"
				...
				<li class="md01"><liferay-ui:message
						key="vgc.apon.second.company.car.expected.car.color" /></li>
				<li class="md022"><input
					id="<portlet:namespace/>expectedCarColor" type="text"
					name="<portlet:namespace/>expectedCarColor" maxlength="75"
					value="<%=secondCompanyCar.getExpectedCarColor()%>" /></li>
			</ul>
			<ul class="rmark">
				<li class="md01"><liferay-ui:message
						key="vgc.apon.second.company.car.preference.for.last.digit.of.plate.number" /></li>
				<li class="md02"><input type="text"
					name="<portlet:namespace/>expectedLastPlateNumber" maxlength="75"
					value="<%=secondCompanyCar.getExpectedLastPlateNumber()%>" /></li>
				<li class="md04 remark"><liferay-ui:message
						key="vgc.apon.second.company.car.remark.info" /></li>
			</ul>
		</div>
...
<aui:form action="<%=applicantAgentURL%>" method="post" name="applicantAgentForm">
	<aui:fieldset>
		<aui:input name="secondCompanyCarId" type="hidden" value="<%=secondCompanyCarId %>"/>
				...
				Liferay.Util.selectEntity(
					{
						...
						uri:'<portlet:renderURL windowState="<%= LiferayWindowState.POP_UP.toString() %>"><portlet:param name="mvcPath" value="/html/admin/select_car.jsp" /><portlet:param name="staffCode" value="<%=String.valueOf(secondCompanyCar.getStaffCode())%>" /><portlet:param name="carType" value="<%= CompanyCarConstants.SECOND_COMPANY_CAR%>" /></portlet:renderURL>'
					},
					function(event) {
